   ...
   void decompose()
    ...
    void generate_exhaustive(int block_index, int prime_index)
    {
        ...
        if (block_index >= block_list->size())
        {
            ...
            printf("floorplan bounding area current : %f \n", objval);
...
//---------------------------------------------
        ...
        r.add(IloNumVar(env, 0, IloInfinity, ILOINT, "Nothing2"));
        y.add(IloNumVar(env, 0, IloInfinity, ILOINT, "Nothing3"));
        w.add(IloNumVar(env, 0, IloInfinity, ILOINT, "Nothing4"));
        h.add(IloNumVar(env, 0, IloInfinity, ILOINT, "Nothing5"));
        ...
        {
            name = "x" + to_string(i);
            x.add(IloNumVar(env, 0, fpga_width, ILOINT, name.c_str()));
            name = "r" + to_string(i);
            r.add(IloNumVar(env, 0, 1, ILOINT, name.c_str()));
            name = "y" + to_string(i);
            y.add(IloNumVar(env, 0, fpga_width, ILOINT, name.c_str()));
//---------------------------------------------
            name = "w" + to_string(i);
            w.add(IloNumVar(env, 0, block_list->at(i - 1)->area, ILOINT, name.c_str()));
            name = "h" + to_string(i);
            h.add(IloNumVar(env, 0, block_list->at(i - 1)->area, ILOINT, name.c_str()));
 //---------------------------------------------
...
/*            hi = block_list->at(i - 1)->height;
            wi = block_list->at(i - 1)->width;
            model.add(h[i]*w[i] = block_list->at(i - 1)->area);*/
            model.add(IloRange(env, block_list->at(i - 1)->area,h[i]*w[i], +IloInfinity, "qc1"));
            model.add(x[i] + h[i] * r[i] - w[i] * r[i] <= fpga_width - w[i]);
            model.add(y[i] - yh <= -h[i]);
        }
        for (int i = 1; i <= block_list->size(); i++)
        {
            for (int j = i + 1; j <= block_list->size(); j++)
...
/*                hi = block_list->at(i - 1)->height;
                ...
                wi = block_list->at(i - 1)->width;
                wj = block_list->at(j - 1)->width;*/
                model.add(x[i] + h[i] * r[i] - w[i] * r[i] - x[j] - M * p[i][j] - M * q[i][j] <= -wi);
                model.add(y[i] + w[i] * r[i] - h[i] * r[i] - y[j] - M * p[i][j] + M * q[i][j] <= M - hi);
                model.add(x[i] - h[j] * r[j] + w[j] * r[j] - x[j] - M *p[i][j] + M * q[i][j] >= -M + wj);
                model.add(y[i] - w[j] * r[j] + h[j] * r[j] - y[j] - M * p[i][j] - M * q[i][j] >= -2 * M + hj);
            }
        }
...
#ifdef CPLEX_USE
        case 1:
            this->solver = &fpga_floorplan::cplex2;
            break;
#endif
#ifdef GUROBI_USE
        case 2:
