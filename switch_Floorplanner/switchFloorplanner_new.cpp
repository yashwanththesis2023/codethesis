   ...
   void decompose()
    ...
    double cplex()
    {
        ...
        for (int i = 1; i <= block_list->size(); i++)
        {
            ...
            name = "y" + to_string(i);
            y.add(IloNumVar(env, 0, 1, ILOFLOAT, name.c_str()));
    ...
    double gurobi()
    {
        ...
        for (int i = 1; i <= block_list->size(); i++)
        {
            x[i] = m1.addVar(0.0, GRB_INFINITY, 0.0, GRB_CONTINUOUS, "x" + to_string(i));
            r[i] = m1.addVar(0.0, 1, 0.0, GRB_INTEGER, "r" + to_string(i));
            y[i] = m1.addVar(0.0, 1, 0.0, GRB_CONTINUOUS, "y" + to_string(i));
